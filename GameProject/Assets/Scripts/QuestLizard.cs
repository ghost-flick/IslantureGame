...
public class QuestLizard : Enemy
{
    ...
    private bool passedDialog;

    void Start()
    {
        dialogs = Directory.GetFiles("Assets/Dialogs/QuestLizard/CommonDialogs", "*.txt")
    ...
    void FixedUpdate()
    {
        ...
        if (direction.magnitude < 0.5 && !passedDialog)
        {
            passedDialog = true;
            dialogState = true;
            animator.SetBool(Move, false);
            DialogManager.Instance.ShowDialog(dialogs.First(), LeaveDialogMove);
        }
    ...
    public void LeaveDialogMove()
    {
        dialogState = false;
    }

    public new IEnumerator RemoveLater()
    {
        dialogState = true;
        DialogManager.Instance.ShowDialog(dialogs.Last(), () =>
        {
            LeaveDialogMove();
            npc.threshold += 1;
